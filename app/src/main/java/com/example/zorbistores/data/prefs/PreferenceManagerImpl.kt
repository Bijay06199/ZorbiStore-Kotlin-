...
open class PreferenceManagerImpl(private val prefs: SharedPreferences):PreferenceManager {
    ...
    override fun setOrderDetails(order: List<LineItem>) {
        prefs[IS_ORDER_DETAILS]=order
    }

    override fun getOrderDetails(): List<LineItem> {
        return prefs[IS_ORDER_DETAILS]!!
    }
    ...
    operator fun SharedPreferences.set(key: String, value: Any?) {
        when (value) {
            ...
            is LineItem->prefs.edit{
                var gson=Gson()
                var json=gson.toJson(value).toInt()
                putInt(key,json)
            }
